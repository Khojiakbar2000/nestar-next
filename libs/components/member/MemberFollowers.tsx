...
const MemberFollowers = (props: MemberFollowsProps) => {
	...
	/** APOLLO REQUESTS **/
	console.log("📦 Sending followInquiry:", followInquiry);
	...
	const {
		...
		refetch: getMemberFollowersRefetch} = useQuery(GET_MEMBER_FOLLOWERS, {
			...
			onCompleted: (data: T) => {
				...
				useEffect(() => {
					if (getMemberFollowersError) {
					  console.error("❌ GraphQL Error:", getMemberFollowersError.message);
					  console.log("🪵 Full Error Object:", getMemberFollowersError);
					}
	...
	useEffect(() => {
		...
		if (id) {
			setFollowInquiry(prev => ({
				...prev,
				search: { followingId: id as string }
			}));
		}
								...
								<Stack className={'details-box'}>
									...
									<Box className={'info-box'} component={'div'}>
										{follower?.meLiked && follower?.meLiked[0]?.myFavorite ? (
											...
											onClick={()=> likeMemberHandler(follower?.followerData?._id,
												getMemberFollowersRefetch, followInquiry
											)} />
