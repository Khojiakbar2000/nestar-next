...
const [updateProperty] = useMutation(UPDATE_PROPERTY)
	const {
		...
		refetch: getAgentPropertiesRefetch} = useQuery(GET_AGENT_PROPERTIES, {
			fetchPolicy: 'network-only',
			variables: {input: searchFilter},
			notifyOnNetworkStatusChange: true,
			/*onCompleted: (data: T) => {
				setAgentProperties(data?.getAgentProperties?.metaCounter[0]?.total ?? 0)
	...
	const updatePropertyHandler = async (status: string, id: string) => {
		try{
			if(await sweetConfirmAlert(`Are you sure to change to ${status}`)){
				await updateProperty({
					variables: {
						input: {
							_id: id,
