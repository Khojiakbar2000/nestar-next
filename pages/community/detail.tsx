...
const device = useDeviceDetect();
	...
	const [boardArticle, setBoardArticle] = useState<BoardArticle>();
		...
		const {
			...
			refetch: boardArticleRefetch} = useQuery(GET_BOARD_ARTICLE, {
				skip: !articleId || articleId.length !== 24,
				fetchPolicy: 'network-only',
				variables: {input: articleId},
				notifyOnNetworkStatusChange: true,
				onCompleted: (data:any) => {
					//setBoardArticle(data?.getProperties?.list);
	...
	const {
		...
		refetch: getCommentsRefetch} = useQuery(GET_COMMENTS, {
			skip: !articleId || articleId.length !== 24,
									...
									<Stack className="content-and-info">
										...
										<Stack className="info">
											<Stack className="icon-info">
												...
												<ThumbUpAltIcon onClick={() => boardArticle?._id && likeBoardArticleHandler(user, boardArticle._id)} />
                    ...
                    <Stack className="top">
                      <Button>
                        {boardArticle?.meLiked && boardArticle?.meLiked[0]?.myFavorite ? (
                          ...
                          <ThumbUpOffAltIcon onClick={() => boardArticle?._id && likeBoardArticleHandler(user, boardArticle?._id)} />
